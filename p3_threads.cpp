...
void *threadfunc(void *param)
{
	...
	while (true) {
		...
		// if no more work to be done leave loop
		if (global_work==0) {
			pthread_mutex_unlock(&mutex);
			break;
		}

		// push id into readyqueue
		readyQue.push_back(id);
		stamp = get_time_stamp();
		printf("[%lu ms][Thread %d] Goto sleep\n", stamp, id);
		
		
		// goes into sleep and waits for a signal from scheduler, unlocks mutex
		while (true){
		...
		// if work is finished signal task is done
		if (global_work==0) { 
			pthread_mutex_lock(&mutex);
			occupied = 0;
			pthread_mutex_unlock(&mutex);
